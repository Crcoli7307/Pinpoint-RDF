...
The interactive map draws an orange array-bearing arrow only when the characterized directional-array option is enabled, every connected antenna has a known orientation, and at least two channels produce a usable amplitude-comparison direction. Pinpoint matches relative channel levels against the configured beamwidth and front-to-back response; this is not phase-coherent angle of arrival. After three spatially distinct fixes provide sufficiently intersecting forward bearings, a green predicted-transmitter marker and oriented uncertainty ellipse are added. Single-antenna, uncharacterized, stationary, low-confidence, parallel, ambiguous, and otherwise underconstrained observations intentionally produce no array arrow or location ellipse.

Vehicle heading is accepted from a fresh GPS course only above the configured minimum speed. At lower speeds, Pinpoint requires displacement greater than both the configured baseline and the receiver-accuracy allowance, smooths accepted headings, and expires stale headings. This prevents stationary GPS jitter from rotating the array solution. GPS alone cannot determine stationary yaw or reliably distinguish vehicle orientation while reversing; those cases require an external heading sensor or operator-supplied orientation.

**Demo Mode**

Demo Mode follows an irregular patrol route and generates complex IQ for each simulated directional receiver. The samples pass through the same signal processing, quality, spectrum, waterfall, amplitude-direction, fusion, and multi-fix intersection paths used by live collection. A repeating scenario introduces RF interference, degraded and lost GPS, an SDR dropout, an insufficient-movement pause, and multipath distortion. The corresponding health states and field alerts appear in the normal interface, making the demo suitable for feature evaluation without presenting its output as real sensor data.
